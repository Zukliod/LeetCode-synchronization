...
# 2. Extract your username dynamically from the action environment
...
def get_recent_github_activity():
    ...
    if not GITHUB_USERNAME:
        print("❌ Missing GitHub repository username configuration.")
        return None

    # Public endpoint to scan your profile activity feed
    url = f"https://github.com{GITHUB_USERNAME}/events/public"
    headers = {
        "Accept": "application/vnd.github.v3+json"
    }
    # Include authentication if token is provided to bypass strict rate limiting
    if GITHUB_TOKEN:
    ...
    try:
        ...
        print(f"❌ Error collecting public metrics: {e}")
        return None
...
def generate_ai_caption(leetcode_count, new_projects):
    ...
    raw_data_summary = f"LeetCode coding problems synced today: {leetcode_count}. New repositories created: {new_projects}."
...
def main():
    ...
    if not linkedin_message:
        status_updates = []
        if leetcode_count > 0:
            status_updates.append(f"✅ Completed {leetcode_count} data structure problems on LeetCode.")
        if new_projects:
